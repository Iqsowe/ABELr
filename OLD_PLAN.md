...
# PLAN — Rework (measurement scale, embedded JPEG, k-NN) (ARCHIVED 2026-07-25)

> Archived in favor of [`PLAN.md`](PLAN.md). All steps closed (S0, Q1-Q7, R1-R5, W1-W5,
> D1-D3, N1-N4, T1-T6) except `T7` (test-coverage backlog) and `G1` (segmentation gate),
> both carried forward into the new plan. Kept here for the measurements, MAE baselines and
> decision rationale — notably: D1's ablation table, D2's `_CALIB_SPREAD_MAX` sweep, N2's
> per-band-vs-global result, W2's real calibration values, R5's live scale/timing evidence.
## Origin
...
- [x] **D3 — Adaptive k-means palette (conditional).** Only if D1 plateaus the MAE. Prior art:
  ...
  **Done 2026-07-25 — gate checked, evidence says don't build.** Unblocked by the same live
  re-analyze as D2 (610-seed pool). Ablated D1 directly: `k_nearest_weighted(..., weights={
  band_frac_<name>: 0.0 for all 8 bands})` (composition OFF) vs. the production default
  (composition ON, weight 1.0), same LOOCV harness, real seeds:

  | Axis | D1 ON (default) | D1 OFF (ablated) |
  |---|---|---|
  | Exposure (target L*) | 2.80 | 3.91 |
  | WB Temperature (K) | 107.6 | 130.3 |
  | WB Tint | 1.44 | 2.04 |
  | Calibration (7 fields) | 0.62–2.04 | 1.69–4.96 |
  | HSL chroma / L* / hue | 3.11 / 4.29 / 2.76 | 4.76 / 6.37 / 4.82 |

  **D1 is not remotely plateaued — it's the single largest MAE lever measured in this whole
  plan so far**, roughly 1.4-2.5× better across every axis with composition features on vs. off
  (and this ablation affects every axis, not just HSL, because `band_frac` feeds the *shared*
  global-match distance that Temp/Tint/Calibration/tone all key off, not just the HSL bands).
  D3's own explicit gate ("do not build before D1 is proven insufficient") therefore fails on
  real data — the opposite of insufficient. **Decision: do not build D3.** k-means palette
  clustering would only be worth the added complexity/timing cost if the *existing* per-band
  `frac` composition signal had run out of room to give — it clearly hasn't. Left gated/closed
  here rather than reopened speculatively; `tools/cluster_sharp_zone.py` stays untouched, no
  code changed by this step (pure evidence-gathering, matches W4's own pattern of a step that
  concludes "leave as-is" rather than building the speculative option). `python -m pytest
  app/tests -q`: unaffected, still 208 passed (no production code touched).
...
- [x] **N4 — Surface confidence in the GUI.** Distinct visible line in `main_window.py`, not
  ...
  **Done 2026-07-25.** The fresh-render chunking refactor (`FreshRenderWorker`/
  ...
  - Step-level test (whole N section): re-run S0 in per-band mode — per-band MAE improved vs.
    ...
    **Done 2026-07-25.** Unblocked by the same live re-analyze as D2 (610-seed pool). LOOCV,
    global match vs. `match_target_per_band` (N2), real HSL bands:
    | Axis | Global | Per-band (N2) |
    |---|---|---|
    | Chroma (C*) | 3.085 (n=1666) | 3.165 (n=1671) |
    | Lightness (L*) | 4.260 (n=1666) | 4.371 (n=1671) |
    | Hue (°) | 2.750 (n=1666) | 2.810 (n=1671) |

    **Per-band matching is consistently, if slightly, worse than the global match on real data —
    not an improvement.** Honest result per this step's own criterion: **N2 stays unwired** in
    `autocorrect.py` (already the case, cf. N2's own note) — `match_target_per_band` remains
    available/tested but the production HSL axis keeps calling `match_target`. Plausible reading:
    this catalog's seeds already cluster tightly by overall scene (same event/lighting), so
    narrowing to a band-specific match mostly trades a slightly larger, more representative
    neighbor pool for a smaller, noisier one — matches the D1 finding that composition features
    barely move things once the base match is already scene-coherent.

    Confidence gate (N3) correlation: nearest-match distance vs. mean per-seed band-L* error,
    n=609, **pearson r=0.241** (real, positive, moderate — distance genuinely tracks error, not
    just noise). At the production default `percentile=75.0`: flagged (low-confidence) seeds mean
    error 5.44 vs. unflagged 3.77 (+44%) — the gate does what it's meant to. `percentile=75.0`
    validated as a reasonable working default; not re-tuned (no evidence a different percentile
    would meaningfully change the flagged/unflagged separation, and this wasn't the step's
    question — settling the exact optimal percentile would need its own sweep, not done here).
